...
@pytest.mark.parametrize("state", [state() for state in states.State.__subclasses__()])
...
def test_schedule_reconnection_is_called_once_network_connectivity_is_detected_only_if_vpn_connection_dropped_and_connection_error_is_not_fatal(
    did_vpn_drop,
    is_connection_error_fatal,
    scheduled_reconnection_expected,
    vpn_connector,
    vpn_data_refresher,
    vpn_monitor,
    network_monitor,
    ...
    with (
        patch.object(
            VPNReconnector, "did_vpn_drop", new_callable=PropertyMock
        ) as did_vpn_drop_patch,
        patch.object(
            VPNReconnector, "is_connection_error_fatal", new_callable=PropertyMock
        ) as is_connection_error_fatal_patch,
        ...
        assert (
            reconnector.schedule_reconnection.called is scheduled_reconnection_expected
        )
...
def test_on_vpn_drop_a_reconnection_attempt_is_scheduled_with_an_exponential_backoff_delay(
    ...
    for number_of_attempts in range(4):
        vpn_monitor.vpn_drop_callback(
            vpn_connector.current_state.context.event
        )  # Simulate VPN drop.

        glib_mock.timeout_add.assert_called_once()
        delay_in_ms, reconnect_func = glib_mock.timeout_add.call_args_list[0].args

        expected_raw = 2**number_of_attempts * 1000
        expected_delay_in_ms = min(
            max(expected_raw, VPNReconnector.MIN_RETRY_DELAY_MS),
            VPNReconnector.MAX_RETRY_DELAY_MS,
        )
...
@pytest.mark.parametrize(
    "is_network_up,is_session_unlocked", [(True, False), (True, False), (False, False)]
)
@patch("proton.vpn.app.gtk.services.reconnector.reconnector.random")
@patch("proton.vpn.app.gtk.services.reconnector.reconnector.GLib")
def test_reconnection_is_rescheduled_if_connection_error_is_not_fatal_when_network_is_down_or_session_is_locked(
    ...
    glib_mock.timeout_add.assert_called_once()
    delay_in_ms, reconnect_func = glib_mock.timeout_add.call_args_list[0].args
    assert delay_in_ms == VPNReconnector.MIN_RETRY_DELAY_MS
...
def test_on_vpn_drop_trigger_force_refresh_after_expired_certificate_event_is_received(
    ...
    async_executor.submit.assert_called_with(
        vpn_data_refresher.force_refresh_certificate
    )


@patch("proton.vpn.app.gtk.services.reconnector.reconnector.GLib")
def test_schedule_reconnection_returns_false_when_max_retry_count_reached(
    glib_mock,
    vpn_connector,
    vpn_data_refresher,
    vpn_monitor,
    ...
    reconnector.retry_counter = VPNReconnector.MAX_RETRY_COUNT

    result = reconnector.schedule_reconnection()
...
@patch("proton.vpn.app.gtk.services.reconnector.reconnector.random")
@patch("proton.vpn.app.gtk.services.reconnector.reconnector.GLib")
def test_on_network_up_does_not_reset_retry_counter(
    glib_mock,
    random_mock,
    vpn_connector,
    vpn_data_refresher,
    ...
    vpn_connector.current_state = states.Error()
    random_mock.uniform.return_value = 1

    reconnector.retry_counter = 3

    network_monitor.network_up_callback()

    assert reconnector.retry_counter == 3
